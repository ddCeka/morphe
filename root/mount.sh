...
log() {
    echo "- $1" >> "/data/data/com.termux/files/home/.local/morphe/logs/mount_log.txt"
}

rm "/data/data/com.termux/files/home/.local/morphe/logs/mount_log.txt"

log "START"

for DIR in /data/local/tmp/morphe/ /data/adb/post-fs-data.d/ /data/adb/service.d/; do
    if [ ! -e $DIR ]; then
        mkdir "$DIR"
        log "$DIR created."
    fi
...
for FILE in "/data/adb/post-fs-data.d/umount_$PKG_NAME.sh" "/data/adb/service.d/mount_$PKG_NAME.sh" "/data/local/tmp/morphe/$PKG_NAME.apk"; do
    if [ -e "$FILE" ]; then
        rm "$FILE"
        log "$FILE deleted."
    fi
...
PATCHED_APP_PATH="/data/local/tmp/morphe/$PKG_NAME.apk"
